...
class NormalizerCompilerPass implements CompilerPassInterface 
{
	...
	{
		if($container->hasDefinition('clio_framework.normalizer.strategy_collection')) {
			$collection = $container->getDefinition('clio_framework.normalizer.strategy_collection');

			foreach($container->findTaggedServiceIds('clio_framework.normalizer.strategy') as $id => $tags) {
